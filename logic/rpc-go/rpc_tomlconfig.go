...
import (
	...
	"github.com/lfxnxf/frame/BackendPlatform/golang/tomlconfig"
...
func GetServiceName() string {
	return getAppServiceName(inits.Default.App, inits.Default.Name)
}
...
func NewConfigToml(path string, v interface{}) (*ConfigToml, error) {
	inits.Init(inits.ConfigPath(path))
	if err := inits.ConfigInstance().Scan(v); err != nil {
		return nil, err
	}
	tomlDefault = &RpcDefaultConfig{}
	if err := inits.ConfigInstance().Scan(tomlDefault); err != nil {
		return nil, err
	}
	toml = &ConfigToml{
...
func (tc *ConfigToml) String(key string) (string, bool) {
	...
	result := inits.ConfigInstance().Get(subKeys...)
	if result == nil {
		return "", false
	}
...
func (tc *ConfigToml) GetServiceClients() []ServerClient {
	...
	for _, client := range tc.defaultConfig.ServerClient {
		...
		sc.ServiceName = getAppServiceName(inits.Default.App, client.ServiceName)
...
func (tc *ConfigToml) Circuit() []inits.CircuitConfig {
