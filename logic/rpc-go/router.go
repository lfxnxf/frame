...
import (
	...
	dutil "github.com/lfxnxf/frame/logic/inits/utils"
	"github.com/lfxnxf/frame/logic/rpc-go/naming"
...
func GetCluster(name string) *upstream.Cluster {
	...
	return inits.Default.Clusters.Cluster(sc.Cluster.Name)
}

func RegisterService(proto string, port int, tags []string) {
	appServiceName := dutil.MakeAppServiceName(inits.Default.App, inits.Default.Name)
	_, err := dutil.Register(inits.Default.Manager, appServiceName, proto, getServiceTags(tags), config.LocalIPString(), port)
	if err != nil {
		panic(err)
	}
}
...
func serviceClient(service string, config RequestOptionIntercace) (ServerClient, error) {
	sc, err := inits.Default.FindServerClient(service)
	if err != nil {
