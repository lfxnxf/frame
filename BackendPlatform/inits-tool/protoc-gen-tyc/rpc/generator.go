...
const (
	...
	clientPkgPath  = "github.com/lfxnxf/frame/logic/inits/rpc/client"
	serverPkgPath  = "github.com/lfxnxf/frame/logic/inits/rpc/server"
	proxyPkgPath   = "github.com/lfxnxf/frame/logic/inits/proxy"
)
...
func (g *micro) Init(gen *generator.Generator) {
	...
	proxyPkg = generator.RegisterUniquePackageName("proxy", nil)
}
...
func (g *micro) GenerateImports(file *generator.FileDescriptor) {
	...
	g.P(proxyPkg, " ", strconv.Quote(path.Join(g.gen.ImportPrefix, proxyPkgPath)))
	g.P(")")
	g.P()
}
...
func (g *micro) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
	...
	g.P("clients ", "map[string]", clientPkg, ".Client")
	g.P("}")
	g.P()

	// NewClient factory.
	g.P("func New", servAlias, " (c ", clientPkg, ".Factory) ", servAlias, " {")
	g.P("if c == nil {")
	g.P(`panic("factory is nil")`)
	g.P("}")
	g.P("service := &", unexport(servAlias), "{")
	g.P("clients: make(map[string]client.Client),")
	g.P("}")

	for _, method := range service.Method {
		endpoint := fmt.Sprintf("%s.%s.%s", serviceName, servName, method.GetName())
		g.P(fmt.Sprintf(`service.clients["%s"] = c.Client("%s")`, endpoint, endpoint))
	}
	...
	// NewClient factory with name.
	g.P("func New", servAlias+"Name", " (name string) ", servAlias, " {")
	g.P("service := &", unexport(servAlias), "{")
	g.P("clients: make(map[string]client.Client),")
	g.P("}")
	...
	for _, method := range service.Method {
		endpoint := fmt.Sprintf("%s.%s.%s", serviceName, servName, method.GetName())
		g.P(fmt.Sprintf(`service.clients["%s"] = proxy.InitRPC(name, "%s")`, endpoint, endpoint))
	}
	g.P("return service")
	g.P("}")
	g.P()
	...
	for _, method := range service.Method {
		...
		g.generateClientMethod(serviceName, servName, method, descExpr)
	}
	...
	for _, method := range service.Method {
		...
		if !method.GetServerStreaming() && !method.GetClientStreaming() {
			g.P(methName, "(ctx ", contextPkg, ".Context, in *", inType, ")", " (*", outType, ", error )")
			continue
		}
	...
	g.P("}")
	g.P(`opts = append(opts, server.HandlerName(`, fmt.Sprintf(`"%s.%s"`, serviceName, origServName), `))`)
	g.P("h := &", unexport(servName), "Handler{hdlr}")
	g.P("if err := s.Handle(s.NewHandler(&", servName, "{h}, opts...)); err != nil {")
	g.P("panic(err)")
	g.P("}")
	g.P("}")
	g.P()
...
func (g *micro) generateClientMethod(reqServ, servName string, method *pb.MethodDescriptorProto, descExpr string) {
	...
	if !method.GetServerStreaming() && !method.GetClientStreaming() {
		...
		g.P("out := new(", g.typeName(method.GetOutputType()), ")")
		g.P("err :=", fmt.Sprintf(`c.clients["%s"].Invoke(ctx, in, out, opts...)`, reqMethod))
...
func (g *micro) generateServerSignature(servName string, method *pb.MethodDescriptorProto) string {
	...
	if !method.GetClientStreaming() && !method.GetServerStreaming() {
		rets = append(rets, "*"+g.typeName(method.GetOutputType()))
	}
...
func (g *micro) generateServerMethod(servName string, method *pb.MethodDescriptorProto) string {
	...
	if !method.GetServerStreaming() && !method.GetClientStreaming() {
		g.P("func (h *", unexport(servName), "Handler) ", methName, "(ctx ", contextPkg, ".Context, in *", inType, ")", "(*", outType, ", error) {")
		g.P("return h.", serveType, ".", methName, "(ctx, in)")
		g.P("}")
		g.P()
		return hname
	}
