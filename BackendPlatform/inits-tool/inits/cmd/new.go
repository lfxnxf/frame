...
import (
	...
	"github.com/lfxnxf/frame/BackendPlatform/inits-tool/inits/internal/goparser"
...
var (
	files = map[int]file{
		...
		_tplTypeAppToml:     {Name: "app/config/test/config.toml", T: _tplRPCAppToml},
...
var newCmd = &cobra.Command{
	Use:   "new [flags] name",
	Short: "Create a new tyc project.",
	Args: func(cmd *cobra.Command, args []string) (err error) {
		if len(args) != 1 {
			return errors.New("requires a project name")
		}
		p.Name = args[0]
		p.Path, err = path.Rel(path.Join(os.Getenv("GOPATH"), "src"), path.Join(p.Dir, p.Name))
		return
	},
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !isInstall("protoc-gen-tyc") {
			...
			err := install("go", p.Dir,
				...
				"install", "github.com/lfxnxf/frame/BackendPlatform/inits-tool/protoc-gen-tyc")
			if err != nil {
				return err
			}
		}
...
func (p *project) genpb(tp string) error {
	...
	err = install(
		"protoc", "", nil,
		fmt.Sprintf("--tyc_out=plugins=%s:%s", tp, outDir),
		"-I="+deps,
