...
import (
	...
	"github.com/lfxnxf/frame/BackendPlatform/jaeger-client-go/gid"
	"github.com/lfxnxf/frame/BackendPlatform/jaeger-client-go/internal/baggage"
	...
	"github.com/lfxnxf/frame/BackendPlatform/jaeger-client-go/utils"
)

var (
	TraceIPEnv, _        = os.LookupEnv("TRACE_IP")
	ReportAddrEnv, _     = os.LookupEnv("TRACE_ADDR")
	SamplingServerEnv, _ = os.LookupEnv("TRACE_SAMPLING_ADDR")
)
...
func NewTracer(
	...
	dockerEnabled := len(TraceIPEnv) != 0
	// Set tracer-level tags
	t.tags = append(t.tags, Tag{key: JaegerClientVersionTagKey, value: JaegerClientVersion})
	if dockerEnabled {
		t.tags = append(t.tags, Tag{key: TracerHostnameTagKey, value: TraceIPEnv})
	}
	if hostname, err := os.Hostname(); err == nil {
		if dockerEnabled {
			t.tags = append(t.tags, Tag{key: TracerDockerHostnameTagKey, value: hostname})
		} else {
			t.tags = append(t.tags, Tag{key: TracerHostnameTagKey, value: hostname})
		}
	}
	if dockerEnabled {
		t.tags = append(t.tags, Tag{key: TracerIPTagKey, value: TraceIPEnv})
		t.hostIPv4, _ = utils.ParseIPToUint32(TraceIPEnv)
	}
	if ip, err := utils.HostIP(); err == nil {
		if dockerEnabled {
			t.tags = append(t.tags, Tag{key: TracerDockerIPTagKey, value: ip.String()})
		} else {
