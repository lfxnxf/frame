...
import (
	...
	"github.com/lfxnxf/frame/BackendPlatform/jaeger-client-go/internal/baggage/remote"
	"github.com/lfxnxf/frame/BackendPlatform/jaeger-client-go/rpcmetrics"
)
...
func (c Configuration) New(
	...
	//FIXME get agent addr from os.Env in docer mode
	if len(jaeger.SamplingServerEnv) != 0 {
		c.Sampler.SamplingServerURL = "http://" + jaeger.SamplingServerEnv + "/sampling"
	}
	if len(jaeger.ReportAddrEnv) != 0 {
		c.Reporter.LocalAgentHostPort = jaeger.ReportAddrEnv
	}
